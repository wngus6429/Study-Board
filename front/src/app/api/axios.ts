...
const getRequestPath = (url?: string) => {
  ...
  try {
    const baseUrl =
      process.env.NEXT_PUBLIC_BASE_URL ?? (typeof window !== "undefined" ? window.location.origin : "http://localhost");
...
instance.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (AUTH_REFRESH_SKIP_PATHS.has(requestPath)) {
      return Promise.reject(error);
    }
