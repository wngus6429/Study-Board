...
interface UseChannelCardStoriesProps {
  channelId: number;
  category: string;
  currentPage: number;
...
export const useChannelCardStories = ({
  channelId,
  category,
  currentPage,
  searchParamsState,
  ...
  viewCount,
  viewMode,
}: UseChannelCardStoriesProps) => {
  return useQuery<ApiResponse>({
    // queryKey는 채널ID, 검색 옵션, 카테고리, 페이지, 추천 랭킹 모드 등에 따라 달라집니다.
    queryKey: searchParamsState
      ? ["channelStories", "cards", channelId, category, currentPage, searchParamsState, recommendRankingMode]
      : ["channelStories", "cards", channelId, category, currentPage, recommendRankingMode],
    // API 호출 함수 (axios를 사용하여 데이터 fetch)
    queryFn: async () => {
      console.log("채널 카드 데이터 호출");
    ...
    enabled: viewMode === "card" && !!channelId,
