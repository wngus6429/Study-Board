...
export default function ChannelNotificationsPage() {
  ...
  const data = allData
    ? {
        ...allData,
        items: allData.items.filter((notification) => notification.type === "channel_post"),
      }
    : null;
  ...
  const handleNotificationClick = async (notification: INotification) => {
    ...
    // 채널 새 게시글 알림 - 게시글로 이동
    if (notification.type === "channel_post" && notification.post?.id && notification.post?.channelSlug) {
  ...
  const formatNotificationMessage = (notification: INotification) => {
    if (notification.type === "channel_post") {
      ...
      return {
        title: `${channelName}에 ${author}님이 새 게시글을 올렸습니다`,
        content: title,
      };
    } else {
      return {
  ...
  if (error) {
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Typography color="error">채널 알림을 불러오는 중 오류가 발생했습니다.</Typography>
      </Container>
    );
  }
      ...
      <Paper sx={{ p: 3 }}>
        ...
        {isLoading ? (
          ...
          <Box py={5} textAlign="center">
            <Typography color="text.secondary">채널 알림이 없습니다.</Typography>
          </Box>
        ) : (
          <>
            <List>
                      ...
                      <ListItemText
                        primary={
                          <Box display="flex" alignItems="center" gap={1}>
                            ...
                            {!notification.isRead && <Chip label="새 알림" size="small" color="secondary" />}
                          </Box>
                        }
                        secondary={
                          <>
