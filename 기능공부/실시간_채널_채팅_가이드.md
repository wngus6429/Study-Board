...
```
❌ GET http://localhost:8888/socket.io?EIO=4&transport=polling 404 (Not Found)
...
```bash
# 브라우저에서 접속
http://localhost:8888        # 서버 상태 페이지
http://localhost:8888/status # API 상태 확인
```
...
```env
# front/.env.local
NEXT_PUBLIC_WEBSOCKET_URL=http://localhost:8888
```
...
```
❌ GET http://localhost:8888/socket.io 404
```
