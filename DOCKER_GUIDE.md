...
| 백엔드       | NestJS 10 (Express)   | 8888                 |
| 데이터베이스 | MySQL 8.0+            | 3306                 |
| WebSocket    | Socket.IO 4.8.1       | 8888 (백엔드와 동일) |
...
ENV NEXT_PUBLIC_BASE_URL=http://localhost:8888
...
services:
  ...
  frontend:
    ...
    ports:
      - "3000:3000"
    environment:
      NEXT_PUBLIC_BASE_URL: http://backend:8888 # 백엔드 접근 URL
...
예시:
  ...
  프론트에서 백엔드 접근 → 'http://backend:8888'
...
- [ ] `http://localhost:8888/api` Swagger 확인
