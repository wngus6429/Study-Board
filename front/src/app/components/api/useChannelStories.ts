...
interface UseChannelStoriesProps {
  channelId: number;
  category: string;
  currentPage: number;
...
export const useChannelStories = ({
  channelId,
  category,
  currentPage,
  searchParamsState,
  ...
  viewCount,
  viewMode,
}: UseChannelStoriesProps) => {
  return useQuery<ApiResponse>({
    // queryKey는 채널ID, 검색 옵션, 카테고리, 페이지, 추천 랭킹 모드 등에 따라 달라집니다.
    queryKey: searchParamsState
      ? ["channelStories", channelId, category, currentPage, searchParamsState, recommendRankingMode]
      : ["channelStories", channelId, category, currentPage, recommendRankingMode],
    // API 호출 함수 (axios를 사용하여 데이터 fetch)
    queryFn: async () => {
      console.log("채널 테이블 데이터 호출");
    ...
    enabled: viewMode === "table" && !!channelId,
