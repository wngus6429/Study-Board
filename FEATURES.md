...
#### 클린 아키텍처 기반 게시판 (학습용)
- ✅ **계층 분리**: Domain, Application, Infrastructure 계층의 완벽한 분리
- ✅ **의존성 역전 원칙(DIP)**: 인터페이스(Port)를 통한 핵심 비즈니스 로직 보호
- ✅ **순수 도메인 객체**: TypeORM 엔티티와 분리된 순수 TypeScript 도메인 모델 사용
- ✅ **어댑터 패턴**: 웹 요청 처리(Controller)와 DB 접근(Repository)을 어댑터로 구현
