...
#### クリーンアーキテクチャベースの掲示板（学習用）
- ✅ **階層分離**: Domain、Application、Infrastructure 階層の完全な分離
- ✅ **依存性逆転の原則 (DIP)**: インターフェース (Port) を通じたコアビジネスロジックの保護
- ✅ **純粋なドメインオブジェクト**: TypeORM エンティティから分離された純粋な TypeScript ドメインモデルの使用
- ✅ **アダプターパターン**: Web リクエスト処理 (Controller) と DB アクセス (Repository) をアダプターとして実装
